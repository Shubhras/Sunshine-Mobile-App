...
//       );
//     }else if (url === 'https://www.iosapptemplates.com/wp-content/uploads/2019/06/empty-avatar.jpg') {
//        setProfileImage(
//         'https://firebasestorage.googleapis.com/v0/b/sun-sign-inc-1e12b.appspot.com/o/pics.png?alt=media&token=c387f3dc-00bc-4535-be43-0d46f3b44bc0',
//       );
//     }  else {
...
const TinderItemCard = (props) => {
  const { url, name, age, school, distance, lastName, undoSwipe } = props;

  const profileImage = useMemo(() => {
    if (!url) return DEFAULT_PROFILE_IMAGE;
    if (url === OLD_DEFAULT_1 || url === OLD_DEFAULT_2) return DEFAULT_PROFILE_IMAGE;
    return url;
  }, [url]);
        ...
        <View style={styles.name_info_container}>
          <View style={styles.userDetailContainer}>
            ...
            {!!school && (
              <View style={styles.txtBox}>
                <Image style={styles.icon} source={Images.schoolIcon} />
                <CustomText style={styles.label}>{school}</CustomText>
              </View>
            )}
