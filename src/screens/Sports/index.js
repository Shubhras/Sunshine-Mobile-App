...
const SPORTS_OPTIONS = [
  { label: 'Football', type: 'football', key: 'football' },
  { label: 'Soccer', type: 'soccer', key: 'soccer' },
  { label: 'Basketball', type: 'basketball', key: 'basketball' },
  { label: 'Tennis', type: 'tennis', key: 'tennis' },
  { label: 'Track', type: 'track', key: 'track' },
  { label: 'Boxing', type: 'boxing', key: 'boxing' },
  { label: 'Gym', type: 'gym', key: 'gym' },
  { label: 'All', type: 'all', key: 'all' },
...
const Sports = ({ route, navigation }) => {
  ...
  const [switchStates, setSwitchStates] = useState([
    SPORTS_OPTIONS.map(() => false),
    SPORTS_OPTIONS.map(() => false),
  ]);
  const mapReduxValueToSwitches = reduxValue =>
    SPORTS_OPTIONS.map(item => item.type === reduxValue);
  // ✅ PRESELECT FROM REDUX
  useEffect(() => {
    const you = reduxSettings?.sports_you || null;
    const seeking = reduxSettings?.sports_seeking || null;
  ...
  const saveData = () => {
    const you =
      SPORTS_OPTIONS.find((_, i) => switchStates[0][i])?.type || null;
    const seeking =
      SPORTS_OPTIONS.find((_, i) => switchStates[1][i])?.type || null;

    const payload = {
      sports_you: you,
      sports_seeking: seeking,
    };

    console.log('Sports Payload 👉', payload);
