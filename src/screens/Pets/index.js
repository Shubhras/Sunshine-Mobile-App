...
const PETS_OPTIONS = [
  { label: 'Dogs', type: 'dogs', key: 'dogs' },
  { label: 'Cats', type: 'cats', key: 'cats' },
...
const Pets = ({ route, navigation }) => {
  ...
  const [switchStates, setSwitchStates] = useState([
    PETS_OPTIONS.map(() => false),
    PETS_OPTIONS.map(() => false),
  ]);
  const mapReduxValueToSwitches = reduxValue =>
    PETS_OPTIONS.map(item => item.type === reduxValue);
  // ✅ PRESELECT FROM REDUX
  useEffect(() => {
    const you = reduxSettings?.pets_you || null;
    const seeking = reduxSettings?.pets_seeking || null;
  ...
  const saveData = () => {
    const you = PETS_OPTIONS.find((_, i) => switchStates[0][i])?.type || null;
    const seeking =
      PETS_OPTIONS.find((_, i) => switchStates[1][i])?.type || null;

    const payload = {
      pets_you: you,
      pets_seeking: seeking,
    };

    console.log('Pets Payload 👉', payload);
