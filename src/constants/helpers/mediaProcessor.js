...
const compressVideo = async sourceUri => {
  RNFS.stat(sourceUri).then(fileInfo => {
    console.log('Original video size:', fileInfo.size)
  }).catch(err => {
    ...
    FFmpegKit.execute(`-i ${sourceUri} -c:v mpeg4 ${processedUri}`).then(
      async session => {
        ...
        if (ReturnCode.isSuccess(returnCode)) {
          RNFS.stat(processedUri).then(fileInfo => {
            console.log('Compressed video size:', fileInfo.size)
          }).catch(err => {
            console.log('Error getting compressed file info:', err)
          })

          resolve(processedUri)
        } else {
          const output = await session.getOutput()
          console.log('FFmpeg failed with output:', output)
          reject(new Error('Video compression failed'))
        }
      },
    ).catch(error => {
      console.log('FFmpeg execution error:', error)
      reject(error)
    })
  })
}
...
export const blendVideoWithAudio = async (
  ...
  FFmpegKit.execute(command).then(async session => {
    ...
    if (ReturnCode.isSuccess(returnCode)) {
      ...
      const output = await session.getOutput()
      console.log('FFmpeg blending failed:', output)
      callback(null)
    }
  }).catch(error => {
    console.log('FFmpeg execution error:', error)
